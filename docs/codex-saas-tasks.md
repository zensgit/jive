...
## 2026-05-18 收口更新：旧 PR 队列已清空

SaaS Beta 主线已经通过 `#144` 合入 `main`，后续 release readiness、
staging、MoneyThings 和 smoke-gate 文档也已继续进入主线。当前远端
`main` 为 `10228185aff3d9f383b15421733c49aad542a734`，对应 Flutter CI
已通过。

2026-05-18 已将仍然 open 的旧 SaaS 栈 PR 统一标注为 superseded by
`#144` 并关闭：
...
当前 GitHub open PR 队列为空。后续 SaaS 相关工作不再从旧 stack 或旧
worktree 继续推进，统一从最新 `main` 新建独立分支，并附带明确的设计、
验证和回滚说明。
...
- notification outbound delivery / provider 集成。
- 任何 E2EE / 密钥管理实现。
- 任何会修改 migration、sync 协议、RLS 或订阅 truth 的事项。
