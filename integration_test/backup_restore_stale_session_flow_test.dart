...
void main() {
  ...
  setUp(() async {
    ...
    rootDir = await Directory.systemTemp.createTemp(
      'jive_integration_backup_restore_',
    );
  ...
  testWidgets(
    ...
    (tester) async {
      final isarDir = Directory(
        '${documentsDir.path}/jive_integration_${DateTime.now().microsecondsSinceEpoch}_${Random().nextInt(1 << 20)}',
      )..createSync(recursive: true);
      ...
      try {
        ...
        final runtimeService = SyncRuntimeService(
          appInstanceId: 'android_e2e_runtime',
        );
