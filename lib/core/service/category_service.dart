...
class CategoryService {
  ...
  static const Map<String, String> _systemParentAliases = {"心情": "表情"};
  ...
  Future<void> _ensureSystemDefaults() async {
    ...
    for (final base in systemIndex.byKey.values) {
      ...
      if (override?.iconOverride == null &&
          _normalizeIconKey(existingCat.iconName) !=
              _normalizeIconKey(base.iconName)) {
        existingCat.iconName = base.iconName;
        changed = true;
      }
      if (override?.colorHexOverride == null &&
      ...
      if (override?.isHiddenOverride == null &&
  ...
  Future<void> _mergeSystemCategories({
    ...
    if (targetParentName != null) {
      final targetParentKey = _buildSystemParentKey(
        targetParentName,
        isIncome ?? false,
      );
    ...
    final sources = candidates
        .where((cat) => sourceNames.contains(cat.name))
        .toList();
    ...
    await isar.writeTxn(() async {
      ...
      if (updatedTxs.isNotEmpty) {
        final transactions = updatedTxs.values.toList();
        TransactionService.touchSyncMetadataForAll(transactions);
        await isar.jiveTransactions.putAll(transactions);
      }
  ...
  // 获取系统默认库 (只读) - 用于分类管理页
  Map<String, Map<String, dynamic>> getSystemLibrary({
    bool isIncome = false,
  ...
  Map<String, Map<String, dynamic>> _mergeSystemLibraries(
    ...
    for (final entry in income.entries) {
      ...
      final existingChildren = List<Map<String, dynamic>>.from(
        (existing['children'] as List<dynamic>? ?? const []).map(
          (e) => Map<String, dynamic>.from(e as Map),
        ),
      );
      final seen = {
        for (final item in existingChildren) item['name'] as String,
      };
      for (final child
          in (entry.value['children'] as List<dynamic>? ?? const [])) {
    ...
    void indexLibrary(
      ...
      for (final entry in lib.entries) {
        ...
        for (final child in children) {
          ...
          final childKey = _buildSystemChildKey(
            parentName,
            childName,
            isIncome,
          );
  ...
  Future<void> _syncCategoryIconsFromSystemLibrary() async {
    final merged = _mergeSystemLibraries(
      kSystemExpenseLibrary,
      kSystemIncomeLibrary,
    );
    ...
    for (final entry in merged.entries) {
      ...
      final parentIcon = _normalizeIconName(
        entry.value['icon'] as String?,
        parentName,
      );
    ...
    for (final cat in categories) {
      ...
      if (cat.parentKey == null) {
        ...
        final parentKey = _normalizeMatchName(
          _systemParentAliases[parentName] ?? parentName,
        );
  ...
  Future<void> _applySystemOverrides() async {
    ...
    for (final override in overrides) {
      ...
      if (override.iconOverride != null &&
          _normalizeIconKey(cat.iconName) !=
              _normalizeIconKey(override.iconOverride!)) {
      ...
      if (override.parentOverrideKey != null) {
        final desiredParentKey =
            override.parentOverrideKey == _noParentOverrideKey
            ? null
            : override.parentOverrideKey;
        if (desiredParentKey == null ||
            categoryByKey.containsKey(desiredParentKey)) {
      ...
      if (override.orderOverride != null &&
  ...
  Future<bool> _migrateSystemCategoriesByName() async {
    ...
    final overridesByKey = {
      for (final override in overrides) override.systemKey: override,
    };
    ...
    void mergeOverride({
      ...
      final preferIncoming =
          existing == null || source.updatedAt.isAfter(existing.updatedAt);
    ...
    for (final parent in parents) {
      ...
      final baseParent = systemIndex
          .parentByMatchName[_parentMatchKey(parent.name, parent.isIncome)];
      if (baseParent != null) {
      ...
      if (aliasName != null) {
        final aliasParent = systemIndex
            .parentByMatchName[_parentMatchKey(aliasName, parent.isIncome)];
        if (aliasParent != null) {
          parentKeyRemap[parent.key] = aliasParent.key;
          keyRemap[parent.key] = aliasParent.key;
          deletedIds.add(parent.id);
          mergeOverride(
            systemKey: aliasParent.key,
            source: parent,
            base: aliasParent,
          );
          continue;
    ...
    for (final child in children) {
      ...
      if (baseByKey != null && baseByKey.parentKey != null) {
        final parentWasRemapped =
            child.parentKey != null &&
            parentKeyRemap.containsKey(child.parentKey);
      ...
      final parent = child.parentKey == null
          ? null
          : parentByKey[child.parentKey!];
      final parentName = parent?.name;
      final resolvedParentName = parentName == null
          ? null
          : (_systemParentAliases[parentName] ?? parentName);
      final baseParent = resolvedParentName == null
          ? null
          : systemIndex.parentByMatchName[_parentMatchKey(
              resolvedParentName,
              child.isIncome,
            )];
      if (baseParent != null) {
        final baseChild =
            systemIndex.childByMatchName[_childMatchKey(
              baseParent.name,
              child.name,
              child.isIncome,
            )];
    ...
    await isar.writeTxn(() async {
      ...
      if (overrideUpdates.isNotEmpty) {
        await isar.collection<JiveCategoryOverride>().putAll(
          overrideUpdates.values.toList(),
        );
      }
      if (overrideDeleteIds.isNotEmpty) {
        await isar.collection<JiveCategoryOverride>().deleteAll(
          overrideDeleteIds.toList(),
        );
      }
    });
  ...
  Future<void> _remapTransactionCategoryKeys(
    ...
    if (updated.isEmpty) return;
    TransactionService.touchSyncMetadataForAll(updated);
    await isar.writeTxn(() async {
  ...
  Future<void> addCategoryFromLib(
    ...
    JiveCategory? parent = await isar
        .collection<JiveCategory>()
        .filter()
        .nameEqualTo(parentName)
        .parentKeyIsNull()
        .findFirst();
  ...
  Future<JiveCategory?> createSubCategory({
    ...
    final baseParentName = isSystem
        ? systemIndex?.parentByKey[parent.key]?.name ?? parent.name
        : parent.name;
    var key = isSystem
        ? _buildSystemChildKey(baseParentName, trimmed, parent.isIncome)
        : _buildChildKey(parent.key, trimmed);
    final existsByKey = await isar
        .collection<JiveCategory>()
        .filter()
        .keyEqualTo(key)
        .findFirst();
  ...
  Future<JiveCategory?> createParentCategory({
    ...
    final systemIndex = isSystem ? _buildSystemIndex() : null;
    var key = isSystem
        ? _buildSystemParentKey(trimmed, isIncome)
        : _buildParentKey(trimmed);
    final existsByKey = await isar
        .collection<JiveCategory>()
        .filter()
        .keyEqualTo(key)
  ...
  Future<void> _syncTransactionsForCategoryDeletion(JiveCategory cat) async {
    ...
    if (updated.isEmpty) return;
    final transactions = updated.values.toList();
    TransactionService.touchSyncMetadataForAll(transactions);
    await isar.jiveTransactions.putAll(transactions);
  }
  ...
  static Widget _applyDetailPreservingTint(
    ...
    if (_isNeutralGray(toned)) {
      toned = _liftGray(
        toned,
        targetLuminance: liftTarget ?? _defaultLiftTarget,
      );
    } else if (soften) {
  ...
  static Color _liftGray(
    Color color, {
    double targetLuminance = _defaultLiftTarget,
  }) {
    ...
    if (_isFileIcon(name)) {
      ...
      return Image.file(
        ...
        errorBuilder: (_, __, ___) =>
            Icon(Icons.category, size: size, color: tonedColor),
      );
    }
    if (_isAssetIcon(name)) {
      final path = _assetIconPath(name);
      final isCategoryIcon = path.startsWith("assets/category_icons/");
      final style = isCategoryIcon
          ? CategoryIconStyleConfig.current
          : CategoryIconStyle.tinted;
      final shouldTint = isCategoryIcon
          ? (forceTinted ||
                style.shouldTintForCategory(isSystemCategory: isSystemCategory))
          : true;
  ...
  Future<void> setCategoryExcludeFromBudget(
    int id,
  ...
  JiveCategory _buildSystemCategoryFromBase(
    ...
    void indexLibrary(
      ...
      for (final parentName in parentNames) {
        ...
        for (final child in children) {
          ...
          childByMatchName[_childMatchKey(parentName, childName, isIncome)] =
              childBase;
  ...
  JiveCategoryOverride? _buildOverrideFromCategory(
    ...
    final override = existing ?? JiveCategoryOverride()
    ...
    override
      ...
      ..iconOverride =
          _normalizeIconKey(category.iconName) !=
              _normalizeIconKey(base.iconName)
          ? category.iconName
          : null
      ...
      ..orderOverride = includeOrder && category.order != base.order
          ? category.order
          : null
      ..isHiddenOverride = category.isHidden != base.isHidden
          ? category.isHidden
          : null
  ...
  JiveCategoryOverride? _buildOverrideSnapshot({
    ...
    snapshot
      ...
      ..iconOverride =
          _normalizeIconKey(source.iconName) != _normalizeIconKey(base.iconName)
          ? source.iconName
          : null
      ..colorHexOverride = _normalizeColorHex(source.colorHex)
      ..parentOverrideKey =
          parentOverrideKey ??
          _computeParentOverrideKey(
            currentParentKey: source.parentKey,
            baseParentKey: base.parentKey,
          )
      ..orderOverride = null
      ..isHiddenOverride = source.isHidden != base.isHidden
          ? source.isHidden
          : null
  ...
  JiveCategoryOverride? _mergeOverridesForMigration({
    ...
    applyIfNeeded(
      ...
      (value) => merged.nameOverride = value,
      merged.nameOverride,
    );
    applyIfNeeded(
      incoming.iconOverride,
      (value) => merged.iconOverride = value,
      merged.iconOverride,
    );
    applyIfNeeded(
      incoming.colorHexOverride,
      (value) => merged.colorHexOverride = value,
      merged.colorHexOverride,
    );
    applyIfNeeded(
      incoming.parentOverrideKey,
      (value) => merged.parentOverrideKey = value,
      merged.parentOverrideKey,
    );
    if (incoming.isHiddenOverride != null &&
        (preferIncoming || merged.isHiddenOverride == null)) {
  ...
  String suggestIconName(String name, {String fallback = "category"}) {
    ...
    bool containsAny(List<String> tokens) {
      return tokens.any(
        (token) => raw.contains(token) || lower.contains(token),
      );
    }
