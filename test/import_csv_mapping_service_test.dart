...
void main() {
  ...
  test('inspect infers mapping from known headers', () {
    const payload =
        '账本,渠道,转入账户,分类路径,一级分类,二级分类,交易时间,金额,手续费,收支类型,备注,标签\n'
        '家庭账本,WeChat,招商银行,餐饮/早餐,餐饮,早餐,2026-03-15 08:00,18.50,1.20,支出,早餐,工作日 早餐';
    ...
    expect(draft.mapping.categoryPathColumnIndex, 3);
    expect(draft.mapping.parentCategoryColumnIndex, 4);
    expect(draft.mapping.childCategoryColumnIndex, 5);
    expect(draft.mapping.dateColumnIndex, 6);
    expect(draft.mapping.amountColumnIndex, 7);
    expect(draft.mapping.serviceChargeColumnIndex, 8);
    expect(draft.mapping.typeColumnIndex, 9);
    expect(draft.mapping.remarkColumnIndex, 10);
    expect(draft.mapping.tagColumnIndex, 11);
  });
  ...
  test('parseWithMapping accepts three-level category path', () {
    const payload =
        '金额,交易时间,账户,分类路径,备注\n'
        '58.00,2026-03-15 09:00,微信,出行 / 私家车 / 加油,周末加油';
