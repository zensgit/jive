...
assert_stdin true "full billing workflow change" $'.github/workflows/saas_full_billing_staging_smoke.yml\n'
assert_stdin true "release candidate workflow change" $'.github/workflows/saas_release_candidate.yml\n'
assert_stdin true "core staging lane self-test change" $'scripts/test_saas_core_staging_lane.sh\n'
...
assert_stdin true "github secrets check script change" $'scripts/check_saas_github_secrets.sh\n'
assert_stdin true "github secrets push script change" $'scripts/push_saas_github_secrets.sh\n'
assert_stdin true "github secrets self-test change" $'scripts/test_saas_github_secrets.sh\n'
...
assert_stdin true "release readiness report change" $'scripts/report_saas_production_release_readiness.sh\n'
assert_stdin true "release readiness report self-test change" $'scripts/test_saas_production_release_readiness_report.sh\n'
